...
class Camera(object):
    ...
    def command_camera(self, command: str) -> None:
        ...
        # Command Camera
        a = '\"' + self.control_cmd_location + "\" /filename "  +  self.save_folder + image_name +  " " + command
        system(a)
    ...
    def show_camera_info(self, filename) -> None:
        self.command_camera("/export "+filename)

    def set_aperture_number(self, aperture_number) -> None:
        self.command_camera("/aperture "+aperture_number)
    
    def set_exposure_comp_number(self, exposure_comp) -> None:
        self.command_camera("/ec "+exposure_comp)
    
    def set_iso_number(self, iso_number) -> None:
        self.command_camera("/iso "+str(iso_number))
    
    def command_test(self) -> None:
        a ='\"' + self.control_cmd_location + "\" C:\\Users\\A21\\Desktop\\DigiCam-master\\pmpm.tcl"
        system(a)

    
    def set_shutter_speed_number(self, shutter, speed) -> None:
        self.command_camera("/shutter "+shutter+" "+speed)
    
    def reset_image_index(self, a):
        self.image_index = a
    ...
    def set_save_folder(self, address):
        self.save_folder = address

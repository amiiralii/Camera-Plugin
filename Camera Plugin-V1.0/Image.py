...
class Image:
    ...
    def __init__(self, img):
        self.img = img
        self.backup = img
        self.previous_red_channel = 0
        self.previous_blue_channel = 0
        self.previous_green_channel = 0
    ...
    def strech_green_channel(self, start=0, end=255):
        ...
        g[g > end] = end
        self.img = cv2.merge([b, g, r])
    def strech_rgb_channel(self, start=0, end=255):
        b, g, r = cv2.split(self.backup)
        g[g< start] = start
        g[g > end] = end
        b[b< start] = start
    ...
    def apply_blue_channel(self):
        b1, g1, r1 = cv2.split(self.backup)
        b2, g2, r2 = cv2.split(self.img)
        self.backup = cv2.merge([b2, g1, r1])
        img = self.img
    def apply_green_channel(self):
        b1, g1, r1 = cv2.split(self.backup)
        b2, g2, r2 = cv2.split(self.img)
        self.backup = cv2.merge([b1, g2, r1])
        img = self.img
    def apply_rgb_channel(self):
        self.backup = self.img
        img = self.img
